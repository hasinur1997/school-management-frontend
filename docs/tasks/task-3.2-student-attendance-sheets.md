...
## Screens / Components
- **Class sheet**: month/date view (`GET /attendance/sheet` or `GET /attendance`) with status badges per day; export if the contract provides it.
- **Student monthly sheet**: calendar/table of one student's month (`GET /students/{id}/attendance`); summary counts (present/absent/late/leave).
...
## Check When Done
- [ ] Class and per-student monthly sheets render with status badges + counts.
- [ ] Student self-view and parent child-switch work.
