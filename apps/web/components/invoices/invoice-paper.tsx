 ...
 *
 * The mock carried fields the backend doesn't model (multiple line items, a
 * discount, guardian/class/roll, an issue date). The real invoice is a single
 * monthly fee for one student, so those are dropped rather than fabricated: one
 * line item (the month's fee), a Total / Paid / Outstanding summary, and the
 * real payment history. Money is rendered from the API's decimal strings via
 * `formatMoney`/`subtractMoney` (no float math).
 */
...
export function InvoicePaper({
  ...
  return (
    ...
    >
      ...
      {/* Line item (a single monthly fee — the backend has no line items) */}
      <div
        style={{
          marginTop: 30,
          border: `1px solid ${HAIRLINE}`,
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 160px" }}>
          <div style={{ padding: "13px 18px", fontSize: 14.5 }}>
            Monthly fee — {period}
          </div>
          <div
            style={{
