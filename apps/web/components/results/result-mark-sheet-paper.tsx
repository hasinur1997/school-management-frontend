...
/** Guilloche border palette — matches the Claude Design theme presets. */
...
export interface ResultMarkSheetPaperProps {
  /**
  ...
  /** Guilloche border colour preset. Defaults to "sage". */
...
export const MARK_SHEET_DOC_WIDTH = 850
...
/**
 * Drop a trailing year from the "(Held in the Month of …)" line when the exam
 * title already carries that same year, so the sheet doesn't print it twice
 * (e.g. title "… Examination, 2016" + month "June - July, 2016" → "June - July").
 * The year is kept when the title doesn't show it, so it's never lost.
 */
function monthWithoutDuplicateYear(examMonth: string, title: string): string {
  const year = examMonth.match(/(\d{4})\s*$/)?.[1]
  if (!year || !title.includes(year)) return examMonth
  return examMonth.replace(/[\s,]*\d{4}\s*$/, "").trim() || examMonth
}
  ...
  return (
    <div style={{ textAlign: "center" }}>
      {signature ? (
        ...
        <div style={{ height: size + 4 }} aria-hidden />
      )}
      <div
        style={{
...
export function ResultMarkSheetPaper({
  schoolName,
  schoolAddress,
  schoolLogo,
  title,
  ...
  return (
    <div
      ...
      style={{ color: SHEET.ink, fontFamily: FONT_SERIF }}
    >
      <div
        style={{
          backgroundColor: theme.pattern,
          backgroundImage: GUILLOCHE_PATTERN,
          padding: 22,
        }}
      >
        <div style={{ position: "relative", background: SHEET.paper, padding: 2 }}>
          <div
            style={{
              position: "absolute",
              inset: 6,
            ...
            <div style={{ position: "relative", zIndex: 2 }}>
              ...
              <div style={{ textAlign: "center", marginTop: 24 }}>
                ...
                {examMonth ? (
                  ...
                  >
                    ...
                    {monthWithoutDuplicateYear(examMonth, title)})
                  </div>
                ) : subtitle ? (
                  <div
                    style={{
                ...
                >
                  {fields.map((field) => (
                    <div key={field.label} style={{ display: "flex" }}>
                      <span style={{ width: 150, flex: "none" }}>{field.label}</span>
                      <span>
                        :&nbsp;&nbsp;
                        <b
                          style={{
                            fontFamily: field.mono ? FONT_MONO : undefined,
                          }}
                        >
                          {hasValue(field.value) ? field.value : EMPTY_VALUE}
                        </b>
                      </span>
                    </div>
                  ))}
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontWeight: 700, fontSize: 15 }}>{gpaLabel} :</span>
                  <span
                    style={{
              ...
              >
                <div>
                  <div>
                    Date of Publication of Result : {publishDate || EMPTY_VALUE}
                  </div>
...
export function ResultMarkSheetFootnote({ className }: { className?: string }) {
  return (
